        ...
        <div className="panel section">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button type="button" className="secondary" onClick={() => setShowClientsSection((prev) => !prev)}>
                {showClientsSection ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
          {showClientsSection ? <div className="tableWrap"><table style={{ minWidth: 520 }}><thead><tr><th>Nombre</th><th>CUIT/DNI</th><th>Telefono</th><th>Direccion</th><th>Accion</th></tr></thead><tbody>{data.clients.map((c) => <tr key={`client-${c.id}`}><td>{c.fullName}</td><td>{c.taxId || "-"}</td><td>{c.phone || "-"}</td><td>{c.address || "-"}</td><td><button type="button" className="secondary" onClick={() => openEditClient(c)}>Editar</button></td></tr>)}</tbody></table></div> : null}
        </div>
